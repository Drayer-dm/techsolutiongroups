<x-layout>
    ...
    <section class="mx-auto w-full max-w-md px-4 sm:px-6">
        <div class="bg-white dark:bg-slate-900 rounder-lg shadow-sm overflow-hidden">
            ...
            @if (session('status'))
                <p class="mx-4 mt-4 rounded-md bg-green-50 px-4 py-2 text-sm text-green-50 px-4 py-2 text-sm text-green-700 sm:mx-6">
                    {{ session('status') }}
                </p>
            @endif

            <form action="POST" action="{{ route('register') }}" class="px-4 py-5 space-y-5 sm:px-6 sm:py-6">
                ...
                <x-molecules.form.field
                    ...
                    label="correo"
                    type="email"
                    placeholder="symphony.no9 in D MINOR Op. 125 'choral'"
                    required
                />
